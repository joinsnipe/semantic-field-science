    ...
    <form className="space-y-3 text-left" onSubmit={handleSubmit}>
      ...
      <label className="flex items-start gap-2 pt-1">
        ...
        <span className="text-[10px] text-gray-400">
          {t("form.privacy")}{" "}
          <a href="/privacy" className="text-[#6d4aff] hover:underline">
            {t("form.privacy.link")}
          </a>
        </span>
      </label>
...
export function DownloadCard({
  ...
  const borderClass =
    variant === "red"
      ? "border-2 border-[#ff1a1a]/70 bg-white hover:bg-[#fff8f8] animate-glow-red"
      : variant === "blue"
        ? "border-2 border-[#00b4ff]/70 bg-white hover:bg-[#f6fbff] animate-glow-blue"
        : "border-gray-100 hover:border-[#6d4aff]/30 hover:shadow-lg hover:shadow-[#6d4aff]/5";
  const hoverColor =
    variant === "red"
      ? "group-hover:text-[#e63946]"
      : variant === "blue"
        ? "group-hover:text-[#0090dd]"
        : "group-hover:text-[#6d4aff]";
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-base font-semibold text-[#1a1a2e] transition-colors ${hoverColor}`}
      >
        {title}
      </span>
